...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/1761721551/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/1761721551')
   open(f"Users/1761721551/phone.csv","w")
if not os.path.exists('data.csv'):
    open("data.csv","w")
APP_ID =  1213736
API_HASH = "df504031dd5224340f5f0e3e46022048"
BOT_TOKEN = ":-5740195758:AAGvmkmeKfRoLDp2m9PMXemUoD26qnPGgZY"
UPDATES_CHANNEL = "https://t.me/yakuzauyeekleme"
OWNER = [1761721551,]
PREMIUM = [1761721551,]
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Üzgünüm efendim, yasaklandınız. İletişim [Destek Grubu](https://t.me/https://t.me/yakuzauyeekleme).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
      except UserNotParticipant:
         await app.send_message(chat_id=message.chat.id, text="**Lütfen Beni Kullanmak İçin Güncel Kanalıma Katılın!\n ve Kontrol etmek için tıklayın /start**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🤖 Güncelleme Kanalına Katılın 🤖", url=f"https://t.me/https://t.me/yakuzauyeekleme")]]), parse_mode="markdown")
         return 1
      except Exception:
         await app.send_message(chat_id=message.chat.id, text="**Bir şeyler ters gitti. İletişim [Destek Grubu](https://t.me/https://t.me/yakuzauyeekleme).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
...
async def login(lel, message):
 try:
   ...
   if a==1:
      return 
   '''if message.from_user.id not in PREMIUM:
      await app.send_message(message.chat.id, f"**Artık Premium Kullanıcı Değilsiniz\nLütfen bir Alt Yazıya Sahip Olun\n200rs ayda\nPm @tweety6r\n\nResmi Grup @https://t.me/yakuzauyeekleme**")
      return'''
   with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
    r=[]
    l=[]
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open("Users/1761721551/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/1761721551/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
         ...
         for pphone in str_list:
            ...
            async for x in client.iter_participants(From, aggressive=True):
               try:
                  ...
                  if (ra-di)>150:
                     await client.disconnect()
                     r+="**\nBotdestek @https://t.me/yakuzauyeekleme**"
